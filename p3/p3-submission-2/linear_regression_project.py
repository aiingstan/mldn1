...
# Ref https://blog.csdn.net/justheretobe/article/details/7632487

def init_identity_matrix(size):
...
def matxMultiply(A, B):
    ...
    if not cln_a == rwn_b:
        raise ValueError
    return [[sum([A[m][n] * B[n][k] for n in range(cln_a)]) for k in range(cln_b)] for m in range(rwn_a)]
...
def augmentMatrix(A, b):
    return [A[n] + (list(b[n])) for n in range(len(b))]
...
print(gj_Solve(A.tolist(),b.tolist()))
...
# 
# $ Ab = \begin{bmatrix}
#     1 & -\frac{5}{2} & -\frac{3}{2} & -\frac{1}{2} \\
#     0 & -4 & 5 & 0 \\
#     0 & \frac{1}{2} & -\frac{3}{2} & \frac{3}{2} \end{bmatrix}$
# 
# $ --> \begin{bmatrix}
#     1 & 0 & -\frac{37}{8} & -\frac{1}{2} \\
#     0 & 1 & -\frac{5}{4} & 0 \\
#     0 & 0 & -\frac{7}{8} & \frac{3}{2} \end{bmatrix}$
#     
# $ --> \begin{bmatrix}
#     1 & 0 & 0 & -\frac{59}{7} \\
#     0 & 1 & 0 & -\frac{15}{7} \\
#     0 & 0 & 1 & -\frac{24}{14} \end{bmatrix}$
#     
# $...$
...
#     0 & 0 & 0 & \frac{5}{3} \\
#     0 & -1 & 1 & \frac{4}{3} \end{bmatrix}$
# 
# $ --> \begin{bmatrix}
#     1 & 0 & \frac{2}{3} & \frac{5}{9} \\
#     0 & 1 & -1 & -\frac{4}{3} \\
#     0 & 0 & 0 & \frac{5}{3} \end{bmatrix}$
#     
#     
# $...$
...
def gj_Solve(A, b, decPts=4, epsilon=1.0e-16):
    ...
    for n in range(cln):
        lead = Decimal(0)
        lead_row = 0
        for m in range(n, rwn):
            if abs(Ab[m][n]) > lead:
                lead = Decimal(abs(Ab[m][n]))
                lead_row = m
        if lead < epsilon:
            return None
        if not lead_row == n:
            swapRows(Ab, lead_row, n)
        scaleRow(Ab, n, 1.0/Ab[n][n])
        for m in range(rwn):
            if not m == n and not Ab[m][n] == 0:
                addScaledRow(Ab, m, n, -1.0*Ab[m][n])
    res = [[Ab[m][cln] for m in range(rwn)]]
...
def linearRegression2D(X,Y):
    argX = [[X[n]] + [1] for n in range(len(X))]
    Y = transpose([Y])
    B = matxMultiply(transpose(argX), Y)
    A = matxMultiply(transpose(argX), argX)
    h = gj_Solve(A, B)
    return float(h[0][0]), float(h[1][0])


# In[122]:


linearRegression2D(X,Y)
